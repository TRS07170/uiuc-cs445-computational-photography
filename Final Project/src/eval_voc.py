...
def voc_eval(
    preds, target, log_file, VOC_CLASSES=VOC_CLASSES, threshold=0.5, use_07_metric=False
):
    """
    ...
    for i, class_ in enumerate(VOC_CLASSES):
        ...
        if len(pred) == 0:  # No predictions made for this class
            ap = 0.0
            log_file.write(
                "---class {} ap {}--- (no predictions for this class)\n".format(
                    class_, ap
                )
            )
            aps += [ap]
...
def evaluate(model, test_dataset_file, img_root, log_file, test_loader=None):
    ...
    log_file.write("---Evaluate model on test samples---\n")
    sys.stdout.flush()
    model.eval()
    ...
    aps = voc_eval(preds, targets, log_file, VOC_CLASSES=VOC_CLASSES)
    return aps
